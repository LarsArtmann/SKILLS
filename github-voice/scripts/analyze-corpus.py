...
def feats(body: str) -> dict:
    return {
        ...
        "bullets": bool(re.search(r"^(\*|-|\d+\.) ", body, re.MULTILINE)),
...
def main() -> None:
    ...
    bodies_ext = [
        i for i in items if i["authored"] and i["repo"].split("/")[0] not in own
    ]
    bodies_own = [
        i
        for i in items
        if i["authored"] and i["repo"].split("/")[0] in own and not is_agent(i["body"])
    ]
    ...
    c_ext = [
        c
        for c in comments
        if c["parent_url"].split("github.com/")[1].split("/")[0] not in own
    ]
    c_own = [
        c
        for c in comments
    ...
    def seg_stats(label: str, texts: list[str]) -> dict:
        ...
        return {
            ...
            "code_fence_pct": pct(sum(x["code_fence"] for x in f), len(f)),
            "inline_code_pct": pct(sum(x["inline_code"] for x in f), len(f)),
            ...
            "openings": dict(Counter(opening_bucket(t) for t in texts).most_common()),
    ...
    analysis = {
        ...
        "edits": {
            ...
            "edit_count_dist": quantiles([e["edit_count"] for e in edits.values()]),
            "with_two_plus_versions": sum(
                1 for e in edits.values() if len(e.get("versions", [])) >= 2
            ),
        },
        "reactions": {
            "comments_mean_reactions": round(
                statistics.mean([c["reactions"] for c in comments]), 2
            ),
            "top_reacted": sorted(comments, key=lambda c: -c["reactions"])[:15],
        },
        "temporal": {
            ...
            "items_per_year": dict(
                sorted(
                    Counter(i["created"][:4] for i in items if i["authored"]).items()
                )
            ),
        },
    }
