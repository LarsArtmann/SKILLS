...
EMOJI_RE = re.compile("[\U0001f000-\U0001faff\u2600-\u27bf\u2b00-\u2bff\u2139\ufe0f]")
...
def check(kind: str, text: str, ai_drafted: bool) -> list[tuple[str, str, str]]:
    ...
    for phrase in FAIL_PHRASES:
        if phrase in low:
            findings.append(
                ("FAIL", "ai-tell phrase", (f'"{phrase}" — 0 hits in corpus'))
            )
    ...
    if DOUBLE_HEDGE_RE.search(text):
        findings.append(
            ("FAIL", "double hedge", "two hedges in one sentence — corpus has zero")
        )
    ...
    if lines:
        ...
        if SIGNOFF_RE.match(last):
            findings.append(("FAIL", "sign-off", f'"{last}" — never signs off'))
    ...
    if kind in ("comment", "review"):
        ...
        if emoji_count >= 3:
            ...
            findings.append(
                ("WARN", "emoji count", "2 emoji — quick comments carry 0-1")
            )
        if len(text) > 600 and not EVIDENCE_RE.search(text):
            findings.append(
                (
                    "WARN",
                    "long comment without evidence",
                    (
