...
class Collector:
        ...
        for attempt in range(10):
            ...
            if proc.returncode == 0:
                ...
                if remaining is not None and remaining < 300:
                    print(
                        f"  GraphQL budget low ({remaining}); "
    ...
    def rest_check_budget(self) -> None:
        ...
        try:
            ...
            if proc.returncode == 0 and proc.stdout.strip():
                ...
                if remaining < 300:
                    print(f"  REST budget low ({remaining}); sleeping 120s", flush=True)
                    time.sleep(120)
    ...
    def search_window(self, qbase: str, start: str, end: str) -> list[dict]:
        ...
        probe = self.gh(
            "-X", "GET", "search/issues", "-f", f"q={q}", "-f", "per_page=1"
        )
        total = probe["total_count"]
        if total == 0:
            return []
        if total > 1000:
            sdate = datetime.strptime(start, "%Y-%m-%d").replace(tzinfo=timezone.utc)
            edate = datetime.strptime(end, "%Y-%m-%d").replace(tzinfo=timezone.utc)
            if (edate - sdate).days >= 2:
                mid = sdate + (edate - sdate) / 2
                mids = mid.strftime("%Y-%m-%d")
                print(f"  split {start}..{end} ({total} hits) at {mids}", flush=True)
                return self.search_window(qbase, start, mids) + self.search_window(
                    qbase, mids, end
                )
            print(
                f"  WARNING: {total} hits in tiny window "
                f"{start}..{end}; taking first 1000",
                flush=True,
            )
        time.sleep(2.1)
        found, page = [], 1
    ...
    def normalize(raw: dict) -> dict:
        repo = raw["repository_url"].replace("https://api.github.com/repos/", "")
        return {
            "node_id": raw["node_id"],
            "url": raw["html_url"],
            "repo": repo,
    ...
    def discover(self, year_start: int) -> None:
        ...
        windows = [
            (f"{y}-01-01", f"{y + 1}-01-01") for y in range(year_start, now_year + 1)
        ]
        queries = [
    ...
    def hydrate(self) -> None:
        commented_other = [
            i for i in self.items.values() if i["commented"] and not i["authored"]
        ]
        auth_comm = [i for i in self.items.values() if i["commented"] and i["authored"]]
        ext_followup = [i for i in auth_comm if not self.own_repo(i["repo"])]
        own_followup = sorted(
        ...
        authored_prs = [
            ...
            if i["authored"]
            and i["is_pr"]
            and not (i["commented"] or i["repo"] in {x["repo"] for x in own_followup})
        ]

        seen: set[str] = set()
        plan = [
            i
            for i in (commented_other + ext_followup + own_followup + authored_prs)
            if not (i["url"] in seen or seen.add(i["url"]))
        ]
        print(
            f"[hydrate] {len(plan)} items "
            f"(threads-by-others {len(commented_other)}, "
            f"ext-followup {len(ext_followup)}, "
            f"own-followup {len(own_followup)}, "
            f"authored-pr {len(authored_prs)})",
            flush=True,
        )
        for n, item in enumerate(plan, 1):
            if n % 100 == 0:
                print(
                    f"  {n}/{len(plan)} ({len(self.comments)} comments so far)",
                    flush=True,
                )
            try:
                self.hydrate_item(item)
            except RuntimeError as exc:
                print(f"  SKIP {item['url']}: {str(exc)[:120]}", flush=True)
            self.rest_check_budget()
    ...
    def fetch_edits(self) -> None:
        ...
        ext_bodies = [i for i in authored if not self.own_repo(i["repo"])]
        own_bodies = sorted(
        ...
        comments = [c for c in self.comments if c["kind"] in ("comment", "review")]
        node2url = {i["node_id"]: i["url"] for i in bodies}
        node2url.update({c["node_id"]: c["html_url"] for c in comments})
        check_ids = list(node2url)
        print(
            f"[edits] checking {len(check_ids)} nodes "
        ...
        for n in range(0, len(check_ids), 50):
            ...
            if n and n % 500 == 0:
                print(
                    f"  checked {n}/{len(check_ids)}, {len(edited_ids)} edited",
                    flush=True,
                )
        print(f"[edits] {len(edited_ids)} items have edits; fetching diffs", flush=True)
        ...
        for n in range(0, len(edited_ids), 25):
            ...
            for node in data.get("nodes") or []:
                ...
                self.edits[node["url"]]["versions"] = sorted(
                    (
                        {"editedAt": v.get("editedAt"), "diff": v.get("diff")}
                        for v in conn.get("nodes", [])
                    ),
                    key=lambda v: v["editedAt"] or "",
                )
                self.edits[node["url"]]["truncated"] = conn.get("totalCount", 0) > 10
    ...
    def load_comments(self) -> None:
        self.comments = []
        data = json.loads((self.out / "raw" / "comments.json").read_text())
        for c in data:
            parent = self.items.get(c["parent_url"])
            if parent:
                self.comments.append(
                    {k: v for k, v in c.items() if k != "parent_url"}
                    | {"parent": parent}
                )
    ...
    def render(self) -> None:
        ...
        for c in self.comments:
            p = c["parent"]
            slug = (
                p["repo"].replace("/", "--") + f"--{p['number']}--{c['node_id'][-10:]}"
            )
            front = [
                f"kind: {c['kind']}",
                f"repo: {p['repo']}",
    ...
    def summary(self) -> dict:
        ...
        stats = {
            ...
            "bodies_authored_issue": sum(1 for i in authored if not i["is_pr"]),
            "bodies_authored_pr": sum(1 for i in authored if i["is_pr"]),
            "bodies_external": sum(1 for i in authored if not self.own_repo(i["repo"])),
            "agent_suspect_bodies": sum(
                1 for i in authored if self.agent_suspect(i["body"])
            ),
            "threads_commented_not_authored": sum(
                1 for i in self.items.values() if i["commented"] and not i["authored"]
            ),
            "comments": len(self.comments),
            "comments_by_kind": dict(Counter(c["kind"] for c in self.comments)),
            "comments_external_repo": sum(
                1 for c in self.comments if not self.own_repo(c["parent"]["repo"])
            ),
            "items_with_edits": len(self.edits),
            "rest_calls": self.rest_calls,
            "graphql_calls": self.gl_calls,
            "top_repos_bodies": dict(
                Counter(i["repo"] for i in authored).most_common(15)
            ),
            "top_repos_comments": dict(
                Counter(c["parent"]["repo"] for c in self.comments).most_common(15)
            ),
        }
        return stats
...
def main() -> None:
    ...
    col = Collector(
        ...
        args.own_followup_sample,
        args.own_edit_bodies,
    )
    ...
    if args.skip_discover:
        ...
        print(
            f"[done-discover] {len(col.items)} items ({time.time() - t0:.0f}s)",
            flush=True,
        )
    if args.skip_hydrate:
        col.load_comments()
        print(f"[resume] loaded {len(col.comments)} comments", flush=True)
    else:
        col.hydrate()
        col.save_json(
            "comments.json",
        ...
        print(
            f"[done-hydrate] {len(col.comments)} comments ({time.time() - t0:.0f}s)",
            flush=True,
        )
    col.fetch_edits()
    ...
    col.render()
    stats = col.summary()
    (out / "summary.json").write_text(json.dumps(stats, ensure_ascii=False, indent=1))
