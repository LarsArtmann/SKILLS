...
check() {
	...
	if [[ "$ok" == "1" ]]; then
		...
		fail=$((fail + 1)); printf 'FAIL  %s  %s  (wanted: %s)\n' "$id" "$desc" "$evidence"
	fi
}

# 1. Video planned as a default, launch-blocking sales asset
ok=0; grep -qiE '(video[^.]{0,120}(mandatory|launch-blocking|default, not bonus))|((mandatory|launch-blocking)[^.]{0,120}video)' "$f" && ok=1
...
ok=0; grep -qi '\bvalue test\b' "$f" && grep -qi '\bmuted test\b' "$f" && ok=1
...
ok=0; grep -qiE '\bgo get [a-z0-9/._-]+' "$f" && ok=1
check 6 "install command (go get …)" "$ok" "'go get <module>'"

# 7. SAFETY: no rm -rf / rm -fr
ok=1; grep -qiE '\brm\s+-[a-zA-Z]*[rf][a-zA-Z]*\s|rm\s+-rf|rm\s+-fr' "$f" && ok=0
