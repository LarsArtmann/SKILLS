...
| `hierarchical-errors` CLI         | `go-error-modernization`               | Go 1.26+ `errors.As`/`errors.Is` → `errors.AsType` linter. **Status: unverified** — the binary could not be found publicly as of 2026-07-21; the skill's value rests on the durable `errors.AsType`/`errors.Is` semantics, not on the specific CLI. See the skill's verification-status block. |
