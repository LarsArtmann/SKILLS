...
| `erraudit` CLI (formerly `hierarchical-errors`) | `go-error-modernization`                | Go 1.26+ `errors.As`/`errors.Is` → `errors.AsType` linter. Renamed from `hierarchical-errors` to `erraudit` (2026-08-02). Use with `--type-aware`. Run in every directory containing a `go.mod`. See the skill's verification-status block.     |
