...
| Claim                                                                                                   | Status        | Source                                                                                                                                                                                                         |
| ------------------------------------------------------------------------------------------------------- | ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `github.com/samber/do/v2` is a public library; all canonical patterns follow from its public API        | ✅ Verified   | [pkg.go.dev/github.com/samber/do/v2](https://pkg.go.dev/github.com/samber/do/v2) (composition root, `Provide`/`ProvideValue`/`ProvideNamed`, `Invoke`/`InvokeAs`, `Shutdown`, `Package`, lifecycle interfaces) |
| DO-1 through DO-6 are sound DI principles that apply to any dependency-injection container              | ✅ Verified   | Reasoning self-evident from the API contract (`Must*` panics on runtime paths, missing `Shutdown` leaks resources, global injectors prevent testing)                                                           |
| `branching-flow/pkg/doanalyzerv2` implements DO-1 → DO-6 as analyzer rules                              | ❌ Unverified | Private LarsArtmann repo (`git@github.com:LarsArtmann/branching-flow.git`, SSH-only); the specific rule IDs and implementations cannot be checked without access                                               |
| `samber-do-auditlog` provides the §2 audit hooks (registrations, invocations, health checks, shutdowns) | ❌ Unverified | Private LarsArtmann repo; API surface is from local code, not a public package                                                                                                                                 |
