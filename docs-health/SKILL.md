...
**Tooling (do not hand-roll):** batch annotations use [./assets/annotate-rows.py](./assets/annotate-rows.py) (numbered table rows) and [./assets/annotate-prose.py](./assets/annotate-prose.py) (numbered prose lists). Both are section-scoped (`## f)` by default / explicit prefix), atomic (write only if every spec matched), refuse already-annotated lines, and support `--dry-run` — ALWAYS dry-run the first spec against a new file shape before mutating (that omission shipped a marker-placement bug on 2026-08-18). annotate-rows scoping is LEVEL-AWARE (upstreamed 2026-09-14): the section ends at the next heading of the same-or-higher level, so `--section "### sub"` annotates inside one sub-section without colliding with sibling `###` tables that repeat row numbers; a `#N` suffix (`--section "### a)#2"`) picks the Nth occurrence of a repeated heading. Spec grammar: `<n>:<kind>:<value>` with kinds `h` (hashes), `v` (verified evidence), `p` (docs-health pass), `w` (won't implement); quote values containing spaces. Completeness gate after an annotate sweep: `grep -rLn '~~' <archived-dir>/` must print NOTHING (every archived file carries at least one strikethrough resolution). Row-level completeness: [./assets/check-rows.py](./assets/check-rows.py) classifies every data row of every table as COMPLETE (uniformly struck), UNTOUCHED, or PARTIAL (mixed struck/unstruck cells or rows — the planted-miss class behind the 2026-09-16 F12.3 miss); exit 1 lists offenders. Run it over every annotated file before declaring a pass done: the grep gate above proves presence, check-rows proves uniformity.
