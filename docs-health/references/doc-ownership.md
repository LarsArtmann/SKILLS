...
| `status-report`   | Produces a point-in-time snapshot. Its \"next tasks\" section is a primary INPUT to `TODO_LIST.md` / `ROADMAP.md` via HARVEST (forward direction, owned by docs-health). The report itself is never rewritten — annotating it to reflect what later shipped is the backward direction, owned by `update-old-docs`. Both directions are needed; neither replaces the other. |
