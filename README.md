...
Skills organized by domain — live count via `scripts/check-skills.sh` (never hardcoded; the checker enforces this). Quality indicators reflect current state: **🟢 Solid** (comprehensive, production-ready), **🟡 Functional** (works but thin), **🔴 Draft** (needs fleshing out), **🆕 New** (structurally valid, never yet triggered against real work — ages into 🟢 after a documented successful run).
...
| **linter-building**          | Build linters, analyzers, and lint-adjacent tools — mechanism decision tree, finding model, FP control, suppressions, autofix, distribution | 🆕 New           |
...
This repository is honest about its state. The authoritative per-skill status inventory is [`FEATURES.md`](FEATURES.md) (coverage enforced by `scripts/check-skills.sh` check 14); the counts below are live-derived, never hardcoded. Most skills are solid, comprehensive, or functional — every established skill has either rich references and templates (the comprehensive ones) or a focused, complete procedure (the solid ones). Five are still **functional** (`architecture-review`, `bdd-testing`, `code-quality-scan`, `deduplicate-code`, `status-report`) — they work and trigger correctly but would benefit from deeper reference material. **Eight skills are 🆕 New** (`buildflow`, `github-voice`, `go-error-modernization`, `jj-fork-pr-workflow`, `linter-building`, `nix-private-go-repos`, `samber-do-best-practices`, `verify-before-filing`) — `collector-extraction` aged to 🟢 Solid after its first live run (ssh-key-monitor, 2026-09-10) — structurally valid and based on real concepts, but not yet triggered against real work. Three of these carry verification-status blocks (`go-error-modernization` for its `erraudit` CLI that is not publicly findable, `nix-private-go-repos` for its private `go-nix-helpers` dependency, `samber-do-best-practices` for its private `branching-flow/pkg/doanalyzerv2` reference) — the skills' value rests on durable concepts, not on the specific private tooling. Validate the repo anytime with `scripts/check-skills.sh` (and its `--selftest` fixture-tests the gate itself).
...
Signal density is machine-checked: `scripts/check-skills.sh --signal` reports preamble size, non-actionable prose blocks, and unglossed jargon per skill (advisory), and check 15 hard-fails pure throat-clearing ("it is important to note" and kin). See `how-to-write-skills.md` Principle 7.
