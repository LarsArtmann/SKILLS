...
**Agent-summarized fetch is not extraction.** When an AI fetch tool *summarizes* a page and reports an exact string — a URL, version, flag, or error message — that string is paraphrase, not evidence. 2026-09-11 incident: three summarizer fetches reported `opengraph.githubassets.com/1/<owner>/<repo>` for a repo whose real og:image (fetched raw later) pointed at `repository-images.githubusercontent.com` — flipping the design conclusion. The claim *felt* verified because a tool produced it. Rule: when the claim is an exact string from a page, fetch the raw bytes and extract mechanically (raw `fetch` + grep, or run the CLI and read its output); never cite a summarizer's rendering of exact bytes.
