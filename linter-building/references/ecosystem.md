...
| Presentation layer        | `~/projects/go-output`                                   | v0.38.x, public                                     | NOT lint-specific: 16 formats × table/tree/graph, CQRS builders→immutable snapshots→pure renderers, NOM progress. Consumed by go-finding's CLI (markdown/csv/tsv via ~20-line `findingToTableData` adapter) and cmdguard. Root is dep-light (only x/term) — the PRO/CONTRA doc in go-finding mandates CLI-only adoption, never core |
| CLI framework             | `~/projects/cmdguard`                                    | v4.0.x, public                                      | NOT lint-specific: Cobra wrapper with struct-tag flags, samber/do DI, zero-panic error contract, lifecycle/shutdown, exit codes. Gives a linter CLI the flags/DI/exit-code shell; pairs with go-output for the 16 formats                                                                                                           |
