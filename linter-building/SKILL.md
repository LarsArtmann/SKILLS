...
description: >
  ...
  static analyzer, custom lint rule, or lint-adjacent tool, or asks which
  linter framework to use. Triggers on "write a linter", "build an analyzer",
  ...
  Covers the full decision tree (configure an existing linter vs delegate to
  a specialist tool vs custom AST walker vs go/analysis vs filesystem rule
  engine vs configurator layer), finding/severity/confidence modeling,
  suppression engineering, autofix safety, false-positive control, testing
  linters, and distribution (CLI, golangci plugin, GitHub Action, SARIF).
  Distinct from code-quality-scan (RUNNING existing linters over a codebase;
  this skill AUTHORS new ones) and how-to-golang (general Go policy).
metadata:
...
| Architecture-test pattern (`go list -deps` + forbidden-pair table) | ✅ verified 2026-09-09 | `InboxClean-lint-baseline/internal/archtest/archtest_test.go` |
| Reason-bearing suppressions in production (`//cqrs-lint:ignore(B022) <reason>`) | ✅ verified 2026-09-09 | `InboxClean-lint-baseline/internal/emailcqrs/setup.go` |
| Converter glue cost 1,200–1,900 LOC per linter; multi-signal + negative filters cut FP ~60%→~0; 158-project validation corpus | ✅ per project docs (research pass 2026-09-09; sub-agent read, key APIs spot-verified) | `go-linter-sdk/README.md`, `go-humanize-linter/docs/` |
| oxlint exits 1 on findings (not an error); oxlint ~841 rules / 113 default-on | ✅ per project docs (research pass 2026-09-09) | `oxlint-auto-configure/README.md`, `pkg/rule/` |

Claims sourced from project docs rather than direct source reads are marked
accordingly; re-verify before relying on exact numbers.
