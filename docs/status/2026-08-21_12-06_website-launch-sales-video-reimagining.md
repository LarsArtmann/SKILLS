...
| -- | -------------------------------------------------------------------------------------------------------------- | ------ | ------ | ------------- |
| ~~1~~  | ~~Fix Phase 6 link-bar split brain: add demo-link variant (or point at readme-template)~~ done — T11 (`2fc1b46` wave): Phase 2 badge/link-bar replaced by a pointer to readme-template (incl. demo variant) | ~~High~~ | ~~S~~ | ~~Bug~~ |
| ~~2~~  | ~~Soften or verify the "`npx` fails on `skills update`" claim in demo-video.md~~ done — wave-2 T5: both HyperFrames claims corrected/softened against the skill bodies (doc-level) | ~~High~~ | ~~S~~ | ~~Quality~~ |
| ~~3~~  | ~~Verify 9:16 re-render claim (composition aspect handling) or mark conditional~~ done — wave-2 T22: 9:16 resized-composition render ffprobe-verified | ~~High~~ | ~~S~~ | ~~Quality~~ |
| ~~4~~  | ~~Read `hyperframes-cli` + `hyperframes-core` SKILL.md; correct routing/tiers claims~~ done — wave-2 T5: hyperframes-cli/core bodies read; routing claims corrected | ~~High~~ | ~~M~~ | ~~Quality~~ |
| ~~5~~  | ~~Add demo-video item to content-patterns.md retrofit checklist~~ done — retrofit checklist now carries the demo-video item (content-patterns.md:229) | ~~High~~ | ~~S~~ | ~~Documentation~~ |
| ~~6~~  | ~~Add CHANGELOG.md entry for this reimagining~~ done — 08-21 CHANGELOG entry backfilled by the 21-25 pass | ~~Medium~~ | ~~S~~ | ~~Documentation~~ |
| ~~7~~  | ~~Run skill-creator eval: 2-3 launch prompts, old-skill baseline vs new~~ done — wave-2 T24: eval-1 re-run on fictional repo (old 4/7 vs new 7/7) + mechanical grader | ~~High~~ | ~~M~~ | ~~Quality~~ |
| ~~8~~  | ~~Trim SKILL.md <800 lines: extract Phase 2 badge/link-bar blocks into readme-template~~ done — T11: 848 → 799 lines (allowlisted) | ~~Medium~~ | ~~M~~ | ~~Cleanup~~ |
| ~~9~~  | ~~Update README.md website-launch row to mention sales-video default~~ done — README row mentions the demo-video sales engine (verified 22-51 a11) | ~~Low~~ | ~~S~~ | ~~Documentation~~ |
| ~~10~~ | ~~Retro-audit existing sites (gogenfilter, go-atomic-write, emeet-pixyd, ...) against the new Definition of Done~~ done — T21 retro-audit executed (2026-09-08_23-15 report) | ~~Medium~~ | ~~M/site~~ | ~~Quality~~ |
| ~~11~~ | ~~Add og:image (1200x630) guidance where OG images are configured (file-manifest/website-creation-details)~~ done — T19: og:image 1200x630 + poster override in file-manifest.md | ~~Medium~~ | ~~S~~ | ~~Documentation~~ |
| ~~12~~ | ~~Add a launch-post copy mini-template to content-patterns.md (hook + link + proof point)~~ done — T19: launch-post mini-template in content-patterns.md | ~~Medium~~ | ~~S~~ | ~~Documentation~~ |
| ~~13~~ | ~~Resolve autoplay-vs-click-to-play policy (question g1) and encode the answer~~ done — routed to ROADMAP Open Questions (autoplay policy, g1 — pre-existing entry) | ~~High~~ | ~~S~~ | ~~Decision~~ |
| ~~14~~ | ~~Resolve social-cut tier policy (question g2) and encode the answer~~ done — routed to ROADMAP Open Questions (social-cut tier, g2 — pre-existing entry) | ~~Medium~~ | ~~S~~ | ~~Decision~~ |
| ~~15~~ | ~~Resolve mandatory-video-for-pure-API-libraries policy (question g3)~~ done — routed to ROADMAP Open Questions (API-library video mandate, g3 — pre-existing entry) | ~~Medium~~ | ~~S~~ | ~~Decision~~ |
| ~~16~~ | ~~Check emeet-pixyd's live site against new placement rules (poster, #demo, og:image)~~ done — T21: emeet audited 4/9; fixes landed wave-3 f5/f6 | ~~Medium~~ | ~~S~~ | ~~Quality~~ |
| ~~17~~ | ~~Add "Watch the demo" mention to Phase 6 badge/link-bar guidance (pairs with #1)~~ done — via T11 pointer (template carries the demo-link variant) | ~~Medium~~ | ~~S~~ | ~~Documentation~~ |
| ~~18~~ | ~~HARVEST this report's (f) into TODO_LIST.md via docs-health (after user instruction)~~ done (docs-health pass 2026-09-18 — harvested by the 21-25 pass and this pass) | ~~Medium~~ | ~~S~~ | ~~Process~~ |
| 19 | Consider a `scripts/` frame-check helper for the sales gates (ffmpeg extract + assert)                         | Low    | M      | Feature       |
| ~~20~~ | ~~ROADMAP fuel: automated "does the landing page video match the README value prop" audit for maintenance mode~~ done — routed to ROADMAP §7 (social-preview/video tooling theme — raw idea) | ~~Low~~ | ~~L~~ | ~~Feature~~ |
