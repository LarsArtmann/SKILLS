...
1. ~~**Testing against realistic prompts** — I loaded the skill-creator skill (which~~ done at `56beebe`
...
2. ~~**Verification of external claims** — The research agents provided extensive~~ done at `85e4b55`, `56beebe`
...
3. ~~**Cross-reference quality** — The go-ecosystem-upgrade → go-release pointer is~~ done at `56beebe`
...
1. ~~**No `evals/` directory** — skill-creator recommends test prompts in~~ done at `56beebe`
   `evals/evals.json`. Not created.

2. ~~**No `scripts/` directory** — The skill could bundle a release-prep script~~ done at `56beebe`
   (check for replace directives, verify no pseudo-versions, run pre-push gates).
   Not created.

3. ~~**No `docs/feedback/new/` scan** — AGENTS.md §11 instructs scanning~~ done (docs/feedback/new/ is empty — nothing to scan (verified 2026-08-21))
...
4. ~~**No `allowed-tools` frontmatter** — AGENTS.md §5.8 suggests pre-approving CLI~~ done at `85e4b55`
   tools. This skill references `goreleaser`, `gh`, `d2` (indirectly). Not set.

5. ~~**No verification-status block** — Several skills carry these for unverified~~ **Won't implement — all external claims verified in rounds 1-2 — status table lives in the 12:10 report section d.**
...
1. ~~**`rm -rf` in SKILL.md Phase 6.2 and in multi-module.md Step 9** — The AGENTS.md~~ done at `85e4b55`
...
2. ~~**`major-versions.md` has a stream-of-consciousness paragraph** — The `+incompatible`~~ done at `85e4b55`
...
3. ~~**Research agent quality was uneven** — The first two agents had **no web access**~~ done at `85e4b55`, `56beebe`
...
4. ~~**SKILL.md at 471 lines is near the 500-line cap** — Leaves almost no room for~~ done at `56beebe`
...
1. ~~**Replace all `rm -rf` with `trash`** — Immediately. Two occurrences in SKILL.md,~~ done at `85e4b55`
   potentially more in reference files. This is a safety rule violation.

2. ~~**Clean up the `+incompatible` paragraph in major-versions.md** — Rewrite as clear~~ done at `85e4b55`
   guidance, remove the "Wait —" thought process.

3. ~~**Verify external claims** — Run the `verify-external-claims` skill against the~~ done at `85e4b55`, `56beebe`
...
4. ~~**Write and run test prompts** — 2-3 realistic "release my Go project" prompts,~~ done at `56beebe`
   run them with and without the skill, compare outputs. The skill-creator skill
   defines this process.

5. ~~**Consider bundling a release-prep script** — A `scripts/pre-release-check.sh` that~~ done at `56beebe`
...
6. ~~**Move Checklist + Gotchas to a reference file** — Frees ~60 lines in SKILL.md,~~ done at `56beebe`
   creating room for future growth without hitting the 500-line wall.

7. ~~**Address the go-ecosystem-upgrade duplication** — Its Phase 6 still has 5 release~~ done at `56beebe`
...
8. ~~**Add `allowed-tools` frontmatter** — Pre-approve `goreleaser` and `gh` so the~~ done at `85e4b55`
   agent doesn't need permission prompts mid-release.

9. ~~**Scan `docs/feedback/new/`** — Check for release-related feedback that should have~~ done (docs/feedback/new/ is empty — nothing to scan)
   informed the skill design.

10. ~~**Pre-release version handling** — The version determination table doesn't mention~~ done at `85e4b55`
...
1. ~~Replace `rm -rf` with `trash` in SKILL.md Phase 6.2 and multi-module.md Step 9~~ done at `85e4b55`
2. ~~Rewrite the `+incompatible` paragraph in major-versions.md (remove "Wait —")~~ done at `85e4b55`
3. ~~Scan all files for other safety-rule violations (NEVER rules from AGENTS.md)~~ done (zero rm in go-release sources after the round-1 sweep)

### Verification (high impact)

4. ~~Verify GoReleaser v2 `version: 2` requirement against live goreleaser.com docs~~ done at `85e4b55`
5. ~~Verify cosign signing args against sigstore docs~~ done at `85e4b55`
6. ~~Verify `go mod edit -retract=v1.2.4` syntax against go.dev/ref/mod~~ done at `85e4b55`
7. ~~Verify SLSA provenance workflow against SLSA framework docs~~ done at `85e4b55`
8. ~~Verify GOPROXY comma-vs-pipe error handling behavior (claimed in research)~~ done at `85e4b55`
9. ~~Run `verify-external-claims` skill against all technical claims in go-release~~ done at `85e4b55`

### Testing (skill-creator process)

10. ~~Write 3 realistic test prompts for go-release~~ done at `56beebe`
11. ~~Run test prompts with skill loaded~~ done at `56beebe`
12. ~~Run test prompts without skill (baseline)~~ done at `56beebe`
13. ~~Compare outputs and iterate on skill weaknesses~~ done at `56beebe`
14. ~~Create `evals/evals.json` with test cases~~ done at `56beebe`

### Structure improvements

15. ~~Move Checklist Summary to `references/checklist.md` or inline it more compactly~~ done at `56beebe`
16. ~~Move Gotchas table to `references/failure-modes.md` (already has R-codes)~~ done at `56beebe`
...
26. ~~Add a section on release branch strategies (main-only vs maintenance branches)~~ done at `56beebe`
...
1. ~~**Should go-ecosystem-upgrade Phase 6 be gutted to a pure pointer to go-release?**~~ done (answered — thinned to 2 ecosystem-specific rules plus a go-release pointer (56beebe))
...
2. ~~**Is the `+incompatible` edge case worth its own section, or is it too rare?**~~ done (answered — kept as a reference section and rewritten in round 1 (85e4b55))
...
3. ~~**Should this skill be project-specific or repo-generic?**~~ done (answered — stayed repo-generic; project specifics live in consumer repos)
