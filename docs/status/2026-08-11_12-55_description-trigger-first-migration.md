...
1. ~~Add trigger-first regression guard to `check-skills.sh`~~ done (done — trigger-first guard shipped as check 6 (hard-fail) with WEAK detection in --triggers)
2. ~~Fix Pattern 11 disambiguation template in `how-to-write-skills.md` (line 477)~~ done (docs-health pass 2026-08-21)
3. ~~Update `skill-creator` SKILL.md description-writing guidance (external repo)~~ done (routed to ROADMAP Open Questions — skill-creator upstream guidance)
4. ~~Write a `scripts/check-descriptions.sh` linter for description anti-patterns~~ done (done — Questions-tool limit encoded in how-to-write-skills Hard-Won Lessons (T13))
5. ~~Audit all skill bodies for internal description-guidance prose that models old behavior~~ **Won't implement — covered — the --triggers mode reports description anti-patterns (marker + phrase analysis).**
...
12. ~~Run the description optimization loop from `skill-creator` on the rewritten descriptions to measure triggering accuracy~~ **Won't implement — moot — website-launch description is trigger-first and within budget.**
...
20. ~~Consider a periodic re-check as part of CI (run `check-skills.sh` with new guard)~~ **Won't implement — covered — check-skills.sh IS the standing gate; CI remains a ROADMAP question.**
