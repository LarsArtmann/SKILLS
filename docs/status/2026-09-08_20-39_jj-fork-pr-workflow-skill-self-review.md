...
| ~~7~~  | ~~Behavioral trigger test: fresh Crush session, realistic prompts ("fork bubbletea and fix X")~~ done — wave-2 T27 (positive+negative fresh-session trigger test passed)             | ~~High~~     | ~~S~~  | ~~Quality~~       |
| ~~8~~  | ~~Read `verify-external-claims` SKILL.md; align verification-block format or define one~~ done — wave-2 T28 (canonical Claim/Status/Source table adopted)                            | ~~Medium~~   | ~~S~~  | ~~Quality~~       |
...
| ~~13~~ | ~~Add a "known upstream skills" note (Carbon-lang jj skill) to the skill's reference file~~ done — wave-2 T29 (prior-art note in SKILL.md + references/prior-art.md)                 | ~~Low~~      | ~~S~~  | ~~Documentation~~ |
...
| ~~15~~ | ~~After first real PR kept green by the sync loop: flip README status 🆕→🟢 with a run note~~ **Won't implement — duplicate — equals TODO_LIST T30 (BLOCKED on the first real PR).** | ~~Low~~      | ~~S~~  | ~~Documentation~~ |
...
| ~~17~~ | ~~Thin skills flagged in inventory: `architecture-visualization` (39), `code-quality-scan` (41), `architecture-review` (54), `bdd-testing` (54) — flesh out per latest audit~~ done — wave-2 T31 (audit re-ran; d2-syntax.md added; 3 of 4 already satisfied)                  | ~~Medium~~ | ~~L~~  | ~~Quality~~       |
| ~~18~~ | ~~§5.6: `how-to-write-skills.md` still at repo root — decide skill-dir vs docs/ move~~ done — routed to ROADMAP Open Questions (location decision, pre-existing)                                                                                                               | ~~Medium~~ | ~~S~~  | ~~Cleanup~~       |
| ~~19~~ | ~~`status-report`/`brutal-self-review` HTML-vs-Markdown override happens often enough to note in their SKILL.md (user frequently demands .md) — consider making .md a documented first-class mode~~ done — routed to ROADMAP Open Questions — 8th recurrence logged 2026-09-18 | ~~Low~~    | ~~S~~  | ~~Documentation~~ |
| ~~20~~ | ~~Consider a repo-level "session start" checklist file (feedback scan + latest status report + AGENTS §8) so agents execute it instead of recalling it~~ done — wave-2 T32 (SESSION-START.md, wired as AGENTS §8 step 0)                                                       | ~~Medium~~ | ~~S~~  | ~~Process~~       |
