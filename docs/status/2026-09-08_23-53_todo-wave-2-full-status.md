...
| -- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------ | ------ | -------------- |
| ~~1~~  | ~~Recreate the two T22 ground-truth compositions as committed fixtures (e.g. `website-launch/assets/demo-compositions/16x9/` + `9x16/`) so the wave report's evidence regains a durable home~~ done — fixtures committed under website-launch/assets/demo-compositions (wave-3 f1, `01b8ad3` wave) | ~~High~~ | ~~S~~ | ~~Documentation~~ |
| ~~2~~  | ~~Extract the HF environment recipe into a runnable `scripts/hf-env.sh` (emits node wrapper + LD_LIBRARY_PATH) and point `demo-video.md` at it instead of prose~~ done — scripts/hf-env.sh shipped (wave-3 f2) | ~~High~~ | ~~S~~ | ~~Feature~~ |
...
| ~~4~~  | ~~T33: redeploy emeet-pixyd hosting; verify `HEAD /demo.mp4` AND one JS asset return `immutable` (closes pitfall #33's live case)~~ done — wave-3 f4: root cause was header ORDER; fixed + live-verified immutable | ~~High~~ | ~~S~~ | ~~Bug (site)~~ |
| ~~5~~  | ~~T33: add `id="demo"` to emeet-pixyd's video container (DoD placement item)~~ done — wave-3 f5: id="demo" live | ~~Medium~~ | ~~S~~ | ~~Bug (site)~~ |
| ~~6~~  | ~~T33: add "Watch the 25s demo" link with accurate runtime to emeet-pixyd README docs bar~~ done — wave-3 f6: README demo badge live | ~~Medium~~ | ~~S~~ | ~~Bug (site)~~ |
...
| ~~12~~ | ~~Make the SKILL.md backlink loop anchor-aware (or delete it in favor of check-skill-links.sh) — one checker, one truth~~ done — wave-3 f12 (anchor-aware backlink loop) | ~~Low~~ | ~~S~~ | ~~Cleanup~~ |
| ~~13~~ | ~~Add a check-skills guard: skills containing verification claims must use the canonical §5 Claim/Status/Source table (warn)~~ done — wave-3 f13 (verification-canon guard, warn) | ~~Medium~~ | ~~S~~ | ~~Quality~~ |
| ~~14~~ | ~~Mechanical grader for eval-1-rerun: regex-file assertions (beat labels, og:image+1200, go get line, rm -rf absence) committed next to grading.json~~ done — wave-3 f14 (mechanical-grade.sh, exact judge agreement) | ~~Medium~~ | ~~S~~ | ~~Quality~~ |
...
| ~~17~~ | ~~Compile-check `bdd-testing/assets/spec-template.go` in a scratch module (never compiled)~~ done — wave-3 f17 (4/4 specs pass, template note added) | ~~Medium~~ | ~~S~~ | ~~Quality~~ |
...
| ~~19~~ | ~~Run ANNOTATE on `2026-09-08_20-39_jj-fork-pr-workflow…` (f7/f8/f13 now done) and on the 08-21 reimagining report's resolved items~~ done (docs-health pass 2026-09-18 — 03-58 rows annotated 2026-09-16; the 08-21_12-06 report annotated by this pass) | ~~Medium~~ | ~~S~~ | ~~Documentation~~ |
| ~~20~~ | ~~Run docs-health HARVEST on this report's (f) into TODO_LIST/ROADMAP after user instruction~~ done (docs-health pass 2026-09-18) | ~~Medium~~ | ~~S~~ | ~~Process~~ |
...
| ~~24~~ | ~~Extend `--triggers` to print WHICH phrases matched per skill (actionable output, not just counts)~~ done — wave-3 f24 (--triggers prints matched phrases) | ~~Low~~ | ~~S~~ | ~~Feature~~ |
| ~~25~~ | ~~Re-run scripts/validate-workflow.sh against the latest jj release (skill promises rerunnability after jj upgrades)~~ **Won't implement — duplicate — wave-2 T31 follow-through closed the d2 gap; validate-workflow rerun tracked via T30.** | ~~Medium~~ | ~~S~~ | ~~Quality~~ |
| ~~26~~ | ~~Schedule the first real upstream PR through the jj workflow (unlocks T30's README status flip)~~ **Won't implement — duplicate — equals TODO_LIST T30 (BLOCKED on the real PR).** | ~~Medium~~ | ~~M~~ | ~~Feature~~ |
...
| ~~33~~ | ~~Session-end checklist: add "no stale git worktrees" + "evidence artifacts committed or disclaimed" to SESSION-START.md~~ done — wave-3 f33 (SESSION-START end rules) | ~~Low~~ | ~~S~~ | ~~Process~~ |
...
| ~~44~~ | ~~Fix the remaining pre-existing AGENTS.md advisory (§5.2 `<--` history note → point at the guard, drop the narrative)~~ done — wave-3 f44 (advisories 2→1) | ~~Low~~ | ~~S~~ | ~~Cleanup~~ |
| ~~45~~ | ~~Verify `ginkgo` CLI presence + `ginkgo unfocus` usage claim on this machine (bdd-testing SKILL.md references it)~~ done — wave-3 f45 (ginkgo v2.32.1 + unfocus verified) | ~~Low~~ | ~~S~~ | ~~Quality~~ |
