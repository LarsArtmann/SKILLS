...
4. **Skill-creator eval loop:** not run; only the repo's heuristic trigger-density check. The skill-creator flow says to *offer* test prompts and let the user decide — I never offered (see g1).
...
4. **Raced the daemon twice** with edits after stale reads — predictable (I had *just documented* the daemon's behavior in the skill I was writing).
