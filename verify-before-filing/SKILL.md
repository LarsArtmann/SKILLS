...
The single most common upstream-issue failure is not a formatting problem or a duplicate — it is a **premise error**: the workaround that motivated the issue was never verified to do anything, so the issue proposes a change that solves no real problem. This skill exists to catch that before you draft a single line.
...
Audit your own configuration first. The most common cause of "upstream doesn't support X" is actually "my override breaks caching or changes behavior for no benefit." Removing an unnecessary override is faster than filing an issue, and it actually fixes the problem.

## 3. Red Flags (Quick Triage)
...
> "The build is slow. Let me upstream the flag so it's cached."
...
>
> **Red flag triage:** The override was recently added by an agent to "enable faster matrix math." Nobody verified whether the target GPU (gfx1150 / RDNA 3.5) even has the relevant instruction units.
>
...
>
> **Gate 3 (Is it relevant to the platform?):** The flag controls MFMA instructions, gated behind `#if defined(CDNA)`. The user's GPU does not define `CDNA`. The flag's code path never executes on their hardware. RDNA uses WMMA (a different instruction set with no toggle). **FAIL.**
>
